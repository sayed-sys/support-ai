    ...
    return (
      <div className={`fixed z-50 ${settings.launcherPosition === 'bottom-left' ? 'bottom-5 left-5' : 'bottom-5 right-5'}`}>
        <button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full text-white shadow-xl hover:scale-105 transition duration-200 focus:outline-none focus:ring-4 focus:ring-blue-300"
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2 max-w-[85%]">
                {!isUser && (
                  <div className="h-7 w-7 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">
                    {isHumanAgent ? 'H' : 'AI'}
                  </div>
                )}
                <div>
                  <div
                    style={isUser ? { backgroundColor: settings.primaryColor || '#4f46e5' } : undefined}
      ...
      <div className="p-3 border-t border-slate-200 bg-white">
        ...
        >
          <input
            ...
            className="flex-1 rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-indigo-500 focus:outline-none transition"
          />
          <button
            type="submit"
